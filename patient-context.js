(async function () {
  ...
  try {
    ...
    const actionRow = view.querySelector('a[href^="note-create"]')?.parentElement;
    if (actionRow) actionRow.insertAdjacentHTML('beforeend', `<a href="diet-plan.html?patientId=${encodeURIComponent(selectedId)}" style="padding:11px 14px;border:1px solid #d9d1c7;color:#051a0f;text-decoration:none;border-radius:7px">Diet plan</a>`);
